...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    setLoadingData(true);
    const { data, error } = await supabase.from('students').select('*').order('code', { ascending: true });
    
    if (data) {
      setStudents(data);
      setFilteredStudents(data);
      // استخراج سنوات الالتحاق بدون تكرار
      const uniqueYears = Array.from(new Set(data.map(s => s.year).filter(Boolean))) as string[];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 dir-rtl text-right" dir="rtl">
      ...
      {/* الترويسة العلوية */}
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl shadow-sm mb-8 border-b-4 border-blue-800">
        <div>
      ...
      {/* شريط الفلاتر وأزرار التصدير */}
      <div className="bg-white p-6 rounded-xl shadow-sm mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-gray-100 text-gray-700 border-b">
                ...
                <th className="p-4 font-bold">كود الطالب</th>
                <th className="p-4 font-bold">الرقم القومي</th>
                <th className="p-4 font-bold">البريد الإلكتروني</th>
                <th className="p-4 font-bold text-center">عام الالتحاق</th>
                <th className="p-4 font-bold text-center">حالة التسجيل</th>
              </tr>
            </thead>
            <tbody>
              {loadingData ? (
                <tr>
                  <td colSpan={7} className="text-center p-8 text-gray-500 font-bold">جاري تحميل البيانات...</td>
                </tr>
                ...
                filteredStudents.map((student, index) => (
                  <tr key={student.nid} className="border-b hover:bg-gray-50 transition">
        ...
        <div className="bg-gray-50 p-4 border-t text-sm text-gray-500 font-bold">
          عرض {filteredStudents.length} من أصل {totalStudents} طالب
        </div>
      </div>
